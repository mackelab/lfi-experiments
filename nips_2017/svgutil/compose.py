...
class Grid(Element):
    ...
    """
    def __init__(self, dx, dy, size=8, width=0.1, xmax=1000, ymax=1000, multiply=1.):
        self.size = size
        self.width = width
        self.xmax = xmax
        self.ymax = ymax
        self.multiply = multiply
        lines = self._gen_grid(dx, dy, self.width)
        element = _transform.GroupElement(lines)
        Element.__init__(self, element.root)

    def _gen_grid(self, dx, dy, width):
        ...
        while x < xmax:
            lines.append(_transform.LineElement([(x, 0), (x, ymax)],
                                                width=width))
            txt.append(_transform.TextElement(x, dy/2, '{:g}'.format(x*self.multiply), size=self.size))
            x += dx
        while y < ymax:
            lines.append(_transform.LineElement([(0, y), (xmax, y)],
                                                width=width))
            txt.append(_transform.TextElement(0, y, '{:g}'.format(y*self.multiply), size=self.size))
            y += dy
        return lines+txt
